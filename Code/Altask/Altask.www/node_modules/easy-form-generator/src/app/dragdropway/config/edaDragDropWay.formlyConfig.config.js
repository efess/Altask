function formlyConfig(formlyConfigProvider, EasyFormGenFormlyProxyModelsProvider, easyFormDragWayConfigProvider) {
    ...
    easyFormDragWayConfigProvider.addControlToDragDropPresentationModel({
        label 	:
          ...
          <div class="form-group">
            ...
            This control is used to position other controls within a line.  A line can accomidate up to three controls.
            </div>
          </div>
      </div>`,
        control : 'empty',
    ...
    formlyConfigProvider.setType({
        name  		: 'html',
        template 	: '<div class="formly-html-content" ng-bind-html="to.label"></div>'
    });
    ...
    },
  {
    ...
    const headerh5Template = `
    <div class="row">
      <div class="">
        <h5 class="text-center">{{options.templateOptions.placeholder}}<h5>
        <hr/>
      </div>
    </div>`;
    ...
    },
  {
    ...
    formlyConfigProvider.setType({ name: 'input-number', template: '<input type="number" class="form-control" ng-model="model[options.key]">', wrapper: ['bootstrapLabel', 'bootstrapHasError']});
...
formlyConfig.$inject = [
  ...
  'EasyFormGenFormlyProxyModelsProvider',
